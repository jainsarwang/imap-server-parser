...
import { listOfMailBox, mailBoxData, noopSuccessType, ParsedMessageSet, SelectOrExamineSuccessType, StoreCommandData, StoreCommandSuccess } from "./commandsType";
...
export interface ImapEmitter {
    ...
    auth: (socket: Socket, authData: {
        username: string;
        password: string;
    }, callback: ImapCallback["auth"]) => void;
    logout: (socket: Socket, callback: ImapCallback["logout"]) => void;
    list: (socket: Socket, requestedPathInfo: {
        reference_name: string;
        mailbox_name: string;
    }, callback: ImapCallback["list"]) => void;
